// Taken from https://github.com/WesUnwin/mtl-file-parser
// under MIT license and updated for the web.
...
class MTLFile {
	...
	parse(defaultMaterialName = "default") {
		...
		lines.forEach((line, index) => {
			...
			switch (lineItems[0].toLowerCase()) {
				...
				default:
					this._notImplemented(`Unrecognized statement: ${lineItems[0]}`);
			}
		});
	...
	_parseIllum(lineItems) {
		...
		this._getCurrentMaterial().illum = Number.parseInt(lineItems[1]);
	}
	...
	_parseKe(lineItems) {
		if (lineItems.length !== 4) {
			this._notImplemented(
				"Ke statements must have exactly 3 arguments (only Ke R G B syntax is supported",
			);
			return;
		}
		const Ke = this._getCurrentMaterial().Ks;
		const color = this._parseKStatementRGB(lineItems);
		Ke.red = color.red;
	...
	_parseKStatementRGB(lineItems) {
		...
		if (lineItems[1].toLowerCase() === "xyz") {
			this._notImplemented("Ka xyz <x> <y> <z>");
			return;
		}

		return {
			red: Number.parseFloat(lineItems[1]),
			green: Number.parseFloat(lineItems[2]),
			blue: Number.parseFloat(lineItems[3]),
		};
	}
	...
	_parseNs(lineItems) {
		...
		this._getCurrentMaterial().shininess = Number.parseFloat(lineItems[1]);
	}
	...
	_parseD(lineItems) {
		...
		this._getCurrentMaterial().dissolve = Number.parseFloat(lineItems[1]);
	}
	...
	_parseTr(lineItems) {
		...
		this._getCurrentMaterial().dissolve = 1.0 - Number.parseFloat(lineItems[1]);
	}
	...
	// map_Ns [options] textureFile
	_parseMapNs(lineItems) {
		// TODO parse options (lineItems[1] to lineItems[lineItems.length - 2])
		if (lineItems.length < 2) {
			this._fileError("to few arguments, expected: map_Ns <textureImageFile>");
		} else if (lineItems.length > 2) {
		}
		const file = lineItems[lineItems.length - 1];
		this._getCurrentMaterial().map_Ns.file = file;
	}
	...
	_parseBump(lineItems) {
		// TODO parse options (lineItems[1] to lineItems[lineItems.length - 2])
		if (lineItems.length < 2) {
			this._fileError("to few arguments, expected: bump <textureImageFile>");
		} else if (lineItems.length > 2) {
		}
		const file = lineItems[lineItems.length - 1];
		this._getCurrentMaterial().map_bump.file = file;
	}

	_parseRefl(lineItems) {
		// TODO parse options (lineItems[1] to lineItems[lineItems.length - 2])
		if (lineItems.length < 2) {
			this._fileError("to few arguments, expected: refl <textureImageFile>");
		} else if (lineItems.length > 2) {
		}
		const file = lineItems[lineItems.length - 1];
		this._getCurrentMaterial().map_refl.file = file;
	}
	...
	_fileError(message) {
		...
		const errorMessage = `==Warning==\nMTL file format error (${line}  ${material}): ${message}\n\nProceeding...`;
		alert(errorMessage);
	}
}
