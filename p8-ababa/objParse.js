// Taken from https://github.com/WesUnwin/obj-file-parser
...
class OBJFile {
	...
	_parseVertexCoords(lineItems) {
		const x = lineItems.length >= 2 ? Number.parseFloat(lineItems[1]) : 0.0;
		const y = lineItems.length >= 3 ? Number.parseFloat(lineItems[2]) : 0.0;
		const z = lineItems.length >= 4 ? Number.parseFloat(lineItems[3]) : 0.0;

		this._currentModel().vertices.push({ x, y, z });
	}

	_parseTextureCoords(lineItems) {
		const u = lineItems.length >= 2 ? Number.parseFloat(lineItems[1]) : 0.0;
		const v = lineItems.length >= 3 ? Number.parseFloat(lineItems[2]) : 0.0;
		const w = lineItems.length >= 4 ? Number.parseFloat(lineItems[3]) : 0.0;
	...
	_parseLine(lineItems) {
		...
		for (let i = 0; i < totalVertices; i += 1) {
			...
			vertexIndex = Number.parseInt(vertexValues[0]);
			if (vertexValues.length > 1 && vertexValues[1] !== "") {
				textureCoordsIndex = Number.parseInt(vertexValues[1]);
			}
	...
	_parsePolygon(lineItems) {
		...
		for (let i = 0; i < totalVertices; i += 1) {
			...
			if (vertexValues.length > 2) {
				vertexNormalIndex = Number.parseInt(vertexValues[2]);
			}
	...
	_parseSmoothShadingStatement(lineItems) {
		...
		const groupNumber =
			lineItems[1].toLowerCase() === "off" ? 0 : Number.parseInt(lineItems[1]);
