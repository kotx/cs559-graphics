...
function parseOBJ(text) {
    ...
    text.split('\n').map(line => {
        ...
        switch (toks[0]) {
            ...
            // Custom directive: scale
            case "x":
                if (toks.length < 3) {
                    throw new Error(`parse-obj: Invalid object scale:${line}`);
                }
                x = [+toks[1], +toks[2], +toks[3]];
                break;

            // Custom directive: rotation
            case "r":
                if (toks.length < 3) {
                    throw new Error(`parse-obj: Invalid object rotation:${line}`);
                }
                r = [+toks[1], +toks[2], +toks[3]];
                break;

            // Custom directive: translation
            case "t":
                if (toks.length < 3) {
                    throw new Error(`parse-obj: Invalid object translation:${line}`);
                }
                t = [+toks[1], +toks[2], +toks[3]];
                break;
